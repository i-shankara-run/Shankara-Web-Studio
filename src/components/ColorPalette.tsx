...
export function ColorPalette({ selectedColor, selectedShade, onSelect }: ColorPaletteProps) {
  const shades = useMemo(() => (selectedColor ? getShades(selectedColor) : []), [selectedColor]);
        ...
        {BRAND_PALETTE.map((c) => {
          ...
          return (
            <button
              ...
              onClick={() => {
                const newShades = getShades(c.hex);
                onSelect(c.hex, newShades[2]);
              }}
